...
class HIL(MessagePassing):
    ...
    def forward(self, x, edge_index_intra, edge_index_inter, pos=None,
        ...
        radial_cov = self.mlp_coord_cov(_rbf(torch.norm(coord_diff_cov, dim=-1).to(torch.float), D_min=0., D_max=6., D_count=9, device=x.device))
        ...
        radial_ncov = self.mlp_coord_ncov(_rbf(torch.norm(coord_diff_ncov, dim=-1).to(torch.float), D_min=0., D_max=6., D_count=9, device=x.device))
