...
mutationName = pd.read_csv('../data/mutationName.csv')['mutation'].tolist()
kpiName = pd.read_csv('../data/kpi2-inter.csv')['kpi'].tolist()

trueResults = {}
with open('../data/resistance.csv', 'r') as csvfile:
...
rvalueDic = {}
spearDic = {}
for idx, kpi in enumerate(kpiName):

    path = f'../results/single/{kpi}_yResult.csv'
    df = pd.read_csv(path)
    ...
    dfNew.to_csv(f'../results/preds/{kpi}_results.csv', index=False)

    dfNew_filter = pd.DataFrame(reformData_filter)
    dfNew_filter.to_csv(f'../results/preds/{kpi}_results_filter.csv', index=False)
...
writePath = f'../results/metrics/a-rmseAll-01.csv'
...
writePath = f'../results/metrics/a-maeAll-01.csv'
with open(writePath, mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(['Key', 'Value'])
...
writePath = f'../results/metrics/a-rValueAll-01.csv'
with open(writePath, mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(['Key', 'Value'])
...
writePath = f'../results/metrics/a-spearAll-01.csv'
with open(writePath, mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow(['Key', 'Value'])
