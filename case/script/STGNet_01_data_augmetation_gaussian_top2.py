...
kpiList = pd.read_csv('../data/kpi2-inter.csv')['kpi'].tolist()
...
for idx_mut, mutName in enumerate(mutationList):
    ...
    for firstKpi in kpiList:
        ...
        path = f'../data/dockEnergyFilterModify/Abl1_{mutName}_relative_meanGaussian.csv'
        df = pd.read_csv(path)
        count = 0
        for secondKpi in kpiList:
            ...
            if expValue == 'nd' or relativeDockList[0] == 'nd':
                ...
                closest_2 = sorted_within_tolerance[:2]
                if len(closest_2) == 2:
                    for item in closest_2:
...
path = '../data/resistance_top2_twin_muts_tuple_noGauss.csv'
