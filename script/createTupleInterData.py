...
def splitData_dat(filePath1, filePath2, filePath3, filePath4):
    ...
    kpi = pd.read_csv(filePath1)['tki'].tolist()
...
def transToDat_All(filePath1, filePath2, filePath3, savePath):
    vocab_path = '../data/initial/seq_char_dict.pkl'
    with open(vocab_path, 'rb') as f:
        seqVoc = pickle.load(f) 

    kpiDf = pd.read_csv(filePath2)
    smilesDic = {} 
    for kpi, smiles in zip(kpiDf['tki'].tolist(), kpiDf['smiles'].tolist()):
        if smiles != 'nd':
...
if __name__ == '__main__':
    filePath1 = '../data/filterData/top5_twin_muts_tuple.csv'
    filePath2 = '../data/initial/tki.csv'
    filePath3 = '../data/initial/mutationSeq.csv'
    savePath = '../data/filterData/top5/dataAll.dat'
    transToDat_All(filePath1, filePath2, filePath3, savePath)
    
    filePath1 = '../data/initial/tki.csv'
    filePath2 = '../data/initial/mutationName.csv'
    filePath3 = '../data/filterData/top5/dataAll.dat'
    filePath4 = '../data/filterData/top5'
